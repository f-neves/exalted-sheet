...
/* ---------------------------------------------------------------- charms */
/*
 * public/charms is generated by tools/build_charms.py, so the checks here are
 * about the seams: that the index and the files agree, that every charm points at
 * a tree that exists, that every tree points at a trait the sheet knows, and that
 * ids are unique, because an id is what a saved sheet stores.
 */
const index = read('public/charms/index.json');
let charmCount = 0;

for (const info of index.sets) {
  const label = `charms/${info.id}`;
  const set = read(`public/charms/${info.id}.json`);
  const text = read(`public/charms/${info.id}.text.json`);
  charmCount += set.charms.length;

  if (set.charms.length !== info.count) {
    errors.push(`${label}: index says ${info.count} charms, the file has ${set.charms.length}`);
  }
  if (set.trees.length !== info.trees) {
    errors.push(`${label}: index says ${info.trees} trees, the file has ${set.trees.length}`);
  }
  if (info.kind === 'native' && !files.includes(`${info.splat}.json`)) {
    errors.push(`${label}: native to "${info.splat}", which is not an exalt type`);
  }
  if (info.kind === 'martial-arts' && !['terrestrial', 'celestial', 'sidereal'].includes(info.tier)) {
    errors.push(`${label}: martial arts with no tier`);
  }

  const treeIds = new Set(set.trees.map((t) => t.id));
  if (treeIds.size !== set.trees.length) errors.push(`${label}: two trees share an id`);
  for (const t of set.trees) {
    if (!t.trait) continue;
    const known = t.traitKind === 'attribute' ? attrIds : abilIds;
    if (!known.has(t.trait)) errors.push(`${label}: tree "${t.id}" names unknown ${t.traitKind} "${t.trait}"`);
  }

  const ids = new Set();
  for (const c of set.charms) {
    if (ids.has(c.id)) errors.push(`${label}: duplicate charm id "${c.id}"`);
    ids.add(c.id);
    if (!c.n) errors.push(`${label}: charm "${c.id}" has no name`);
    if (!treeIds.has(c.t)) errors.push(`${label}: charm "${c.id}" sits in unknown tree "${c.t}"`);
    for (const trait of Object.keys(c.min?.traits || {})) {
      if (!abilIds.has(trait) && !attrIds.has(trait)) {
        errors.push(`${label}: charm "${c.id}" needs unknown trait "${trait}"`);
      }
    }
    for (const pre of c.preIds || []) {
      if (!set.charms.some((x) => x.id === pre)) {
        errors.push(`${label}: charm "${c.id}" points at missing prerequisite "${pre}"`);
      }
    }
  }

  for (const id of Object.keys(text)) {
    if (!ids.has(id)) errors.push(`${label}: rules text for "${id}", which is not a charm here`);
  }
  const silent = set.charms.filter((c) => !text[c.id]).length;
  if (silent > set.charms.length / 10) {
    warnings.push(`${label}: ${silent} of ${set.charms.length} charms have no rules text`);
  }
}
...
console.log(`data ok — ${files.length} splats, ${abilities.length} abilities, ${attributes.length} attributes`
  + `, ${charmCount} charms in ${index.sets.length} sets${warnings.length ? `, ${warnings.length} warning(s)` : ''}`);
